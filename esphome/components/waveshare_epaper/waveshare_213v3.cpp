...
void WaveshareEPaper2P13InV3::dump_config() {
  LOG_DISPLAY("", "Waveshare E-Paper", this);      // Added ;
  ESP_LOGCONFIG(TAG, "  Model: 2.13inV3");         // This one was already correct
  LOG_PIN("  CS Pin: ", this->cs_);                // Added ;
  LOG_PIN("  Reset Pin: ", this->reset_pin_);      // Added ;
  LOG_PIN("  DC Pin: ", this->dc_pin_);            // Added ;
  LOG_PIN("  Busy Pin: ", this->busy_pin_);        // Added ;
  LOG_UPDATE_INTERVAL(this);                       // Added ;
}
